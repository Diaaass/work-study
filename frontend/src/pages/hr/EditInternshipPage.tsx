...
export default function EditInternshipPage() {
    ...
    internshipsApi.getById(id)
      .then((internship) => {
        setForm({
          ...
          type: internship.workType as InternshipType || InternshipType.Remote,
          description: internship.description || '',
          duration: internship.duration || '',
          salary: internship.salary ? String(internship.salary) : '',
          deadline: internship.deadline ? internship.deadline.slice(0, 10) : '',
          requirements: internship.requirements?.length ? internship.requirements : [''],
          skills: internship.skills || [],
        });
        setLogoUrl(internship.companyLogo || '');
      })
      .catch(() => {
        showToast('Ошибка загрузки стажировки', 'error');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      showToast('Стажировка обновлена', 'success');
      navigate('/hr/internships');
    } catch (err) {
      const apiErr = err as ApiError;
      if (apiErr.fieldErrors) setErrors(apiErr.fieldErrors);
      setServerError(apiErr.message || 'Ошибка при сохранении');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>Редактировать стажировку</h1>
        <p className={styles.subtitle}>Измените данные и сохраните</p>
      </div>
      ...
      <form className={styles.form} onSubmit={handleSubmit} noValidate>
        ...
        <div className={styles.formFooter}>
          <Button type="button" variant="secondary" onClick={() => navigate('/hr/internships')}>
            {t('post.cancel')}
          </Button>
